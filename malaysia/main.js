...
Apify.main(async () => {
    ...
    const cheerioCrawler = new Apify.CheerioCrawler({
        ...
        handlePageFunction: async ({ request, $, body }) => {
            ...
            switch (label) {
                case 'GET_IFRAME':
                    const iframUrl = $('header script').attr('id').match(/(?<=_)[^_]+$/g)[0];
                    await requestQueue.addRequest({
